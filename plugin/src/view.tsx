...
function NodepadApp({ plugin, initialData, fileName, folderPath, onSave, onMenuClick, portalContainer, synthesisTriggerRef }: NodepadAppProps) {
  ...
  const generateGhostNote = useCallback(async () => {
    ...
    const context = buildGhostContext(enrichedBlocks).map(b => ({
      id: b.id, text: b.text, category: b.category ?? "general",
    }))

    try {
      const result = await generateGhost(plugin, context, lastGhostTexts)
      setLastGhostTexts(prev => [...prev, result.text].slice(-10))
      setGhostNotes(prev => prev.map(n =>
        n.id === ghostId ? { ...n, text: result.text, category: result.category, isGenerating: false } : n
      ))
    } catch (e: unknown) {
      setGhostNotes(prev => prev.filter(n => n.id !== ghostId))
      const msg = e instanceof Error ? e.message : String(e)
      if (msg.includes("No API key")) new Notice("No API key set — open Settings → Nodepad")
    } finally {
  ...
  const doEnrich = useCallback(async (
    ...
    setBlocks(current => current.map(b => b.id === id ? { ...b, statusText: "Enriching…" } : b))
    ...
    try {
      ...
      setBlocks(current => {
        const mergeTargetId = data.mergeWithIndex !== null
          ? context[data.mergeWithIndex ?? -1]?.id
          : null
  ...
  const handleToggleSubTask = useCallback((blockId: string, subTaskId: string) => {
    setBlocks(prev => prev.map(b => b.id === blockId ? {
      ...b,
      subTasks: b.subTasks?.map((st: { id: string; text: string; isDone: boolean; timestamp: number }) =>
  ...
  // ── Workspace stub (single file = single space) ───────────────────────────
  ...
  const handleCommand = useCallback((cmd: string, text?: string) => {
    ...
    if      (cmd === "kanban") setViewMode("kanban")
      ...
      plugin.app.vault.create(exportPath, md)
        ...
        .catch(() => {
          copyToClipboard(md)
          new Notice("Copied to clipboard (file already exists)")
        })
    }
    else if (cmd === "copy-md") {
      const md = exportToMarkdown(fileName ?? "Nodepad", blocksRef.current)
